...
class SHA256:
    ...
    def pre_processing(self, msg) -> list[int]:
    ...
    @lru_cache
    def expanded_msg_blocks(self, block, index_j):
        if index_j <= 15:
            ...
            return ((Logical().sig1(self.expanded_msg_blocks(block, index_j - 2)) +
                     Logical().sig0(self.expanded_msg_blocks(block, index_j - 15)))
                    + self.expanded_msg_blocks(block, index_j - 16)) % 2 ** 32

    def sha256(self):
        """
        given a message outputs its sha256 hash
        """
        blocks = self.pre_processing(self.msg)
        ...
        for i in range(1, len(blocks) + 1):
            ...
            for j in range(63 + 1):
                W = self.expanded_msg_blocks(blocks[i - 1], j)
            ...
            for j, var in enumerate([a, b, c, d, e, f, g, h]):
                hash_values_i[j] = (var + hash_values[i - 1][j]) % (2 ** 32)
